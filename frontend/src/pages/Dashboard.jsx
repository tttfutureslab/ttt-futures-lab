...
function AccountRulesBlock({ account }) {
  ...
  const hasBestDay = account.last_snapshot?.best_day_pnl !== null && account.last_snapshot?.best_day_pnl !== undefined && account.last_snapshot.best_day_pnl > 0;

  if (!hasConsistencyRule && !hasDdInfo && !hasProfitTarget && !hasMinDays && !hasBestDay) return null;
  ...
  const ddColor = (() => {
    if (!hasDdInfo) return '';
    const limit = Number(account.rule_trailing_dd || 0);
    if (limit > 0) {
      const pct = (account.dd_remaining_usd / limit) * 100;
  ...
  const daysColor = hasMinDays && account.days_remaining > 0 ? 'value-warn' : 'value-pos';
