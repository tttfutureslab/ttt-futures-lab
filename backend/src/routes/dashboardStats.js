 ...
 * PnL agregado por trader y periodo, calculado DIRECTAMENTE desde tabla trades.
 * Ya no usa snapshots. Suma trades.pnl_usd filtrado por fecha.
 */
router.get('/stats', async (req, res) => {
  try {
    // Una sola query agrupada por trader
    ...
    const merged = tradesStats.rows.map((s) => ({
      ...s,
      payouts: payoutMap[s.trader] || {
        payouts_total: 0, payouts_day: 0, payouts_week: 0,
        payouts_month: 0, payouts_year: 0, payouts_count: 0
      }
    }));
...
router.get('/payouts', async (req, res) => {
  try {
    const result = await query(`
      SELECT p.id, p.amount_usd, p.gross_amount, p.payout_date,
...
router.get('/account-types', async (req, res) => {
  try {
    const result = await query(`
      SELECT pf.slug AS firm_slug, pf.name AS firm_name,
             at.type_name, at.display_name
